...
Drift_correction_data = [SMACM_data(:,1) SMACM_data(:,2) SMACM_data(:,3) SMACM_data(:,4), SMACM_data(:,5); fiducial_localizations(:,1) (fiducial_localizations(:,2))+100000 (fiducial_localizations(:,3))+100000 fiducial_localizations(:,4)+100000 fiducial_localizations(:,1)];
...
xBead = 23+629 %76+628; %65+628 %76+628;%px
yBead = 21+629 %75+628; %77+628 %75+628;%px
...
[xBeadFiltered(:,2),xBeadFiltered(:,1)] = filterlocs(xBead(:,2),xBead(:,1),2000,1,'x','\\smb.rdf.rice.edu\research\ag134\Epi in a chip\Data\05 Multi-target imaging with flow\251206 3D Nanobody\Drift_Correction\DC_p3_vim\xBeadFiltered.mat');
[yBeadFiltered(:,2),yBeadFiltered(:,1)] = filterlocs(yBead(:,2),yBead(:,1),2000,1,'y','\\smb.rdf.rice.edu\research\ag134\Epi in a chip\Data\05 Multi-target imaging with flow\251206 3D Nanobody\Drift_Correction\DC_p3_vim\yBeadFiltered.mat');
[zBeadFiltered(:,2),zBeadFiltered(:,1)] = filterlocs(zBead(:,2),zBead(:,1),2000,1,'z','\\smb.rdf.rice.edu\research\ag134\Epi in a chip\Data\05 Multi-target imaging with flow\251206 3D Nanobody\Drift_Correction\DC_p3_vim\zBeadFiltered.mat');
%% Track Fiducial bead 
...
Formatted_DriftCorrectedSMACMData = [frames xnmDriftCorrected_tab ynmDriftCorrected_tab znmDriftCorrected_tab  Drift_correction_data(:,5) probe];
Formatted_DriftCorrectedSMACMData.Properties.VariableNames = {'frames', 'xnm','ynm','znm','intensity','probe'};
